...
from donkeycar.parts.velocity import VelocityNormalize, VelocityUnnormalize, StepSpeedController, PIDSpeedController
from donkeycar.parts.sensordata import ArduinoSpeedReader
...
# Set up console handler to display logs in the terminal
console_handler = logging.StreamHandler()
console_handler.setLevel(logging.INFO)  # Log level for console output
formatter = logging.Formatter('%(asctime)s - %(message)s')  # Log format without logger names
console_handler.setFormatter(formatter)

# Only add the console handler if no handlers are already attached
if not logger.hasHandlers():
    logger.addHandler(console_handler)  # Ensure the handler is added only once

# Configure DonkeyCar parts (e.g., velocity.py) to log speed updates
velocity_logger = logging.getLogger('donkeycar.parts.velocity')
velocity_logger.setLevel(logging.INFO)  # Allow INFO-level logging for velocity

# Prevent adding the console handler again in the velocity_logger
if not velocity_logger.hasHandlers():
    velocity_logger.addHandler(console_handler)  # Ensure it also uses the console handler

velocity_logger.propagate = False  # Prevent duplicate logging
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[]):
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        # Add the joystick controller to the vehicle
        V.add(ctr, 
            inputs=['cam/image_array'], 
            outputs=['user/angle', 'normalized_speed', 'user/mode', 'recording'], 
            threaded=True)

    # Add ArduinoSpeedReader to the vehicle
    arduino_reader = ArduinoSpeedReader(port='/dev/ttyUSB0', baudrate=115200)
    V.add(arduino_reader, outputs=['speed'])  # Registering speed as output from ArduinoSpeedReader

    # Normalize speed using VelocityNormalize
    MIN_SPEED = 0.2  # Minimum car speed (stall speed)
    MAX_SPEED = 3.0  # Maximum car speed
    #velocity_normalize = VelocityNormalize(min_speed=MIN_SPEED, max_speed=MAX_SPEED)
    #V.add(velocity_normalize, inputs=['speed'], outputs=['normalized_speed'])

    # Map joystick throttle to target speed using VelocityUnnormalize
    velocity_unnormalize = VelocityUnnormalize(min_speed=MIN_SPEED, max_speed=MAX_SPEED)
    V.add(velocity_unnormalize, inputs=['normalized_speed'], outputs=['target_speed'])

    # Add the PIDSpeedController to the pipeline
    #pid_speed_controller = PIDSpeedController(kp=1.1, ki=0.07, kd=0.0)
    #V.add(pid_speed_controller, 
    #    inputs=['user/throttle', 'speed', 'target_speed'], 
    #    outputs=['user/throttle'])

    # Add speed controller to match target speed
    speed_controller = StepSpeedController(min_speed=MIN_SPEED, max_speed=MAX_SPEED)
    V.add(speed_controller, 
           inputs=['user/throttle', 'speed', 'target_speed'], 
           outputs=['user/throttle'])

        # Update RoboHAT to use adjusted throttle
        #robohat = RoboHATDriver(cfg)
        #V.add(robohat, inputs=['user/angle', 'adjusted_throttle'])
    ...
    velocity_logger.info(f"Current Speed: 0.0, Target Speed: 0.0")
